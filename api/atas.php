...
try {
    ...
    while (true) {
        ...
        foreach ($lote as $rec) {
            $k = (isset($rec['numeroCompra']) ? $rec['numeroCompra'] : '') . '|' . (isset($rec['anoCompra']) ? $rec['anoCompra'] : '') . '|' . (isset($rec['numeroAtaRegistroPreco']) ? $rec['numeroAtaRegistroPreco'] : (isset($rec['numeroAta']) ? $rec['numeroAta'] : '')) . '|' . (isset($rec['idCompra']) ? $rec['idCompra'] : (isset($rec['id']) ? $rec['id'] : ''));
            if (!isset($seen[$k])) {
